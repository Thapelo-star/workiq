...
import { Card, CardTitle, PageHeader, FormGroup, inputStyle, Table, Td, Badge } from '@/components/ui'
...
function getWeeks(logs: TimeLog[]) {
  ...
  logs.forEach(l => {
    ...
    byWeek[key] = (byWeek[key] || 0) + Number(l.hours)
  })
...
function getDays(logs: TimeLog[]) {
  if (!logs.length) return []
  const byDay: Record<string, number> = {}

  logs.forEach(l => {
    byDay[l.date] = (byDay[l.date] || 0) + Number(l.hours)
  })
...
function getProjectMap(logs: TimeLog[]) {
  const m: Record<string, number> = {}
  logs.forEach(l => { m[l.project] = (m[l.project] || 0) + Number(l.hours) })
  return Object.entries(m).sort((a, b) => b[1] - a[1])
}

function groupLogsByDay(logs: TimeLog[]) {
  ...
  return Object.entries(grouped)
    ...
    .map(([date, items]) => ({
      date,
      total: items.reduce((sum, item) => sum + Number(item.hours), 0),
      items: items.sort((a, b) => a.project.localeCompare(b.project)),
    }))
}

function BarChart({ data, maxVal, color }: { data: { label: string; value: number }[]; maxVal: number; color: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 150, paddingBottom: 28, position: 'relative' }}>
      {data.map((d, i) => {
...
export default function TrendsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (elevated) {
      ...
      const { data: logs } = await supabase.from('time_logs').select('*, profiles(name,team)').limit(3000)
      setAllLogs(logs || [])
      setScope('company')
    } else {
      const { data: logs } = await supabase.from('time_logs').select('*, profiles(name,team)').eq('user_id', user.id).limit(1000)
      setAllLogs(logs || [])
      setScope('mine')
    }
  ...
  const filteredLogs = (() => {
    ...
    if (scope === 'user') {
      return selectedUser ? allLogs.filter(l => l.user_id === selectedUser) : []
    }

    if (scope === 'team') {
      return selectedTeam ? allLogs.filter(l => {
  ...
  const days = getDays(filteredLogs)
  const cats = getCategoryMap(filteredLogs)
  const teams = getTeamTrend(filteredLogs, allProfiles)
  const projects = getProjectMap(filteredLogs)
  const groupedDays = groupLogsByDay(filteredLogs)

  const totalHours = filteredLogs.reduce((s, l) => s + Number(l.hours), 0)
  const activeDays = new Set(filteredLogs.map(l => l.date)).size
  const avgWeekly = weeks.length > 0 ? parseFloat((weeks.reduce((s, w) => s + w.total, 0) / weeks.length).toFixed(1)) : 0
  const avgPerDay = activeDays > 0 ? parseFloat((totalHours / activeDays).toFixed(2)) : 0
  ...
  if (loading) {
    return <div style={{ padding: 48, textAlign: 'center', color: '#a0a8c0', fontSize: 13 }}>Loading trends...</div>
  }

  return (
    <div>
      <PageHeader title="Trends" subtitle="Visual analysis of logged time and activity patterns." />
                ...
                {([
                  ...
                  <button
                    ...
                    style={{
                      fontFamily: 'inherit',
                      fontSize: 13,
                      fontWeight: scope === s.key ? 700 : 500,
                      padding: '7px 16px',
                      borderRadius: 8,
                      border: 'none',
                      cursor: 'pointer',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(155px,1fr))', gap: 14, marginBottom: 24 }}>
        {[
          ...
          { label: 'Avg / Active Day', value: avgPerDay + 'h', color: '#059669' },
          ...
          { label: 'Active Days', value: activeDays.toString(), color: '#dc2626' },
        ].map(s => (
      ...
      {filteredLogs.length === 0 ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            ...
            <Card>
              <CardTitle>Daily Hours (last 21 active days)</CardTitle>
              <BarChart data={days.map(d => ({ label: d.label, value: d.total }))} maxVal={Math.max(...days.map(d => d.total), 1)} color="#0ea5e9" />
            </Card>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            ...
            <Card>
              ...
              {projects.slice(0, 8).map(([project, hours], i) => {
            ...
            <Card style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                <div>
                  ...
                  <StatRow label="Avg per Active Day" value={avgPerDay + 'h'} />
                  <StatRow label="Active Weeks" value={weeks.length.toString()} />
                  <StatRow label="Active Days" value={activeDays.toString()} />
          ...
          <Card>
            <CardTitle>Daily Log Detail</CardTitle>
            {groupedDays.map(day => (
              <div key={day.date} style={{ marginBottom: 20, border: '1px solid #eef1f7', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ background: '#f8f9fc', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#1a1d2e' }}>{fmtDate(day.date)}</div>
                  <div style={{ fontSize: 12, color: '#6b7280', fontFamily: 'DM Mono,monospace' }}>{day.total.toFixed(2)}h</div>
                </div>

                <Table heads={['Person', 'Project', 'Category', 'Task', 'Duration']} empty={day.items.length === 0}>
                  {day.items.map(item => (
                    <tr key={item.id}>
                      <Td>{(item.profiles as any)?.name || '-'}</Td>
                      <Td>{item.project}</Td>
                      <Td><Badge text={item.category} /></Td>
                      <Td style={{ maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.task}</Td>
